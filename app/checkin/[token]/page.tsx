...
export default async function CheckinPage({
  ...
  // Look up token — allow already-used tokens so we can show a nice message
  const { data: tokenRow } = await service
    .from('patient_checkin_tokens')
    .select('patient_id, clinic_id, expires_at, used_at')
    .eq('token', token)
    .single()
  ...
  // Already used — show today's score if available
  if (tokenRow.used_at) {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-background to-surface/30 px-5 py-10">
      <div className="mx-auto max-w-lg space-y-8">
        ...
        <CheckinForm token={token} patientFirstName={patient.first_name} />
      </div>
    </main>
  )
}
